...
import { Scraper, ExtendedScraper } from '../base/Scraper';
...
export class CodeChefScraper
  implements Scraper<CcUrlParams>, ExtendedScraper<CcSubmissions>
{
  ...
  private async getToken() {
    ...
    }>(CodeChefScraper.API_ENDPOINT + '/oauth/token', {
  ...
  getUrlParams(url: string): CcUrlParams {
    ...
    for (const pattern of CodeChefScraper.urlPatterns) {
  ...
  async fetchUserSubmissions(handle: string, contestId?: string) {
    ...
    for (let i = 1; i <= 1000; ++i) {
      const apiUrl = new URL(`${CodeChefScraper.API_ENDPOINT}/submissions`);
      ...
      if (typeof data === 'string' || data.result.data.code === 9003) break;
